...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->rtbOrnekler->Size = System::Drawing::Size(256, 96);
			...
			this->rtbAgirliklar->Size = System::Drawing::Size(256, 42);
			...
			this->button1->Location = System::Drawing::Point(369, 229);
			this->button1->Name = L"button1";
			this->button1->Size = System::Drawing::Size(94, 38);
			this->button1->TabIndex = 15;
			this->button1->Text = L"Son Noktayi Sil";
			...
			this->ClientSize = System::Drawing::Size(740, 377);
...
private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
	if (referansNoktalar != nullptr) {
		Nokta* n = referansNoktalar->getNoktalar();
		if (n->getSonraki() == nullptr) { //Bos
			MessageBox::Show("Alinmis ornek yok", "Uyari!");
			rtbOrnekler->Clear();
			return;
		}
		while (n->getSonraki()->getSonraki() != nullptr) {
			n = n->getSonraki();
		}
		
		int sonuncuSatir = sinifBirSayac+sinifIkiSayac -1;
		/*rtbOrnekler->Lines[sonuncuSatir] = "";
		rtbOrnekler->Refresh();*/

		int start_index = rtbOrnekler->GetFirstCharIndexFromLine(sonuncuSatir);
		int count = rtbOrnekler->Lines[sonuncuSatir]->Length;

		// Eat new line chars
		if (sonuncuSatir < rtbOrnekler->Lines->Length - 1)
		{
			count += rtbOrnekler->GetFirstCharIndexFromLine(sonuncuSatir + 1) -
				((start_index + count - 1) + 1);
		}

		rtbOrnekler->Text = rtbOrnekler->Text->Remove(start_index, count);


		if (n->getSonraki()->getSinifNo() == SINIF_BIR)
		{
			sinifBirSayac--;
			lSinifBirSayac->Text = sinifBirSayac.ToString();
		}
		if (n->getSonraki()->getSinifNo() == SINIF_IKI)
		{
			sinifIkiSayac--;
			lSinifIkiSayac->Text = sinifIkiSayac.ToString();
		}
		delete n->getSonraki();
		n->setSonraki(nullptr);
	}
	pictureBox1->Refresh();
}
};
